...
@section('content')
    ...
    <div>
        <a style="width: 114px;height:40px;padding-top:12px" class="btn btn-primary" href="{{ url('/buy') }}">Back</a>
    </div>
        ...
        <table class="table">
            ...
            <tbody id="body">
                <tr style="color: white">
                    ...
                    <td><input type="number" class="unit"  name="unit[]" placeholder="unit"></td>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $(function() {
                // $('#unit1, #price1').keyup(function(){
                //    var value1 = parseFloat($('#unit1').val()) || 0;
                //    var value2 = parseFloat($('#price1').val()) || 0;
                //    $('#tprice1').val(value1 * value2);
                // });

                // Target all unit and price inputs with classes
                $('#body').on('keyup', '.unit, .price', function() {
                    var currentRow = $(this).closest('tr'); // Get the current row
                    var unitValue = parseFloat(currentRow.find('.unit').val()) || 0;
                    var priceValue = parseFloat(currentRow.find('.price').val()) || 0;
                    currentRow.find('.tprice').val(unitValue *
